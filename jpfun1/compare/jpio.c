...
JPEGBuffer *jpio_read(char *file_name){
  ...
  // Set normal error routines
  cinfo.err = jpeg_std_error(&jerr);  
  // Create decomp object  
  ...
  // Create a new JPEG buffer
  JPEGBuffer *result = jpbuf_create(cinfo.output_width, cinfo.output_height,
  ...
  for (int i=0;i<cinfo.output_height;i++) {
   ...
   // Copy the row to our buffer
   memcpy(result->image_buffer + i*row_stride,row_buffer[0],row_stride);
